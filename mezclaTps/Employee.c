...
//**************************************************************************************************************
int employee_map(LinkedList* pArrayList)
{
    int retorno = 0;
    char seguir  = 'n';
    if(pArrayList != NULL)
    {
        ll_map(pArrayList,aumentoDeSueldo);
        if (pArrayList != NULL )
        {
            employee_list(pArrayList);
            printf("\nDesea continuar trabajando con esta lista? S/N: ");
            printf("\nADVERTENCIA: Si ingresa si, los datos actuales se sobreescribiran por los nuevos ");
            printf("\n Si ingresa no, todos los cambios no guardados hasta el momento se perderan");
            fflush(stdin);
            scanf("%c", &seguir);
            fflush(stdin);
            ...
            {
                controller_loadFromText("data.csv",pArrayList);
            }else
            {
                printf("\nSe recomienda al usuario guardar los cambios hechos antes de seguir modificando el archivo.");
            }
        }
        retorno = 1;
    }
...
//***********************************************************************************************
 int aumentoDeSueldo(Employee* pElement )
 {
    int retorno = 0;
    int porcentajeAgregar;
    if(pElement != NULL)
    {
        porcentajeAgregar = pElement->sueldo * 20;
        porcentajeAgregar = porcentajeAgregar / 100;
        pElement->sueldo = pElement->sueldo + porcentajeAgregar;
        retorno = 1;
    }
    return retorno;
 }
//***********************************************************************************************
int employee_filter(LinkedList* pArrayList)
{
    int retorno = 0;
    char seguir  = 'n';
    ...
    if(pArrayList != NULL)
    {
        cloneLinked = ll_filter(pArrayList,selectoPorHoras);
        if (cloneLinked != NULL )
        {
            employee_list(cloneLinked);
            printf("\nDesea continuar trabajando con esta lista? S/N: ");
            printf("\nADVERTENCIA: Si ingresa si, los datos actuales se sobreescribiran por los nuevos ");
            printf("\n Si ingresa no, todos los cambios no guardados hasta el momento se perderan");
            fflush(stdin);
...
//****************************************************************************
int selectoPorHoras(Employee* pElement)
{
    int retorno = 0;
    if(pElement != NULL)
    {
        if(pElement->horasTrabajadas >= 100)
        {
            retorno = 1;
        }
    }
...
//***************************************************************************
int employee_reduce(LinkedList* pArrayList)
{
    int retorno = 0;
    char seguir  = 'n';
    if(pArrayList != NULL)
    {
        ll_reduce(pArrayList,reducirPorSueldo);
        if (pArrayList != NULL )
        {
            employee_list(pArrayList);
            printf("\nDesea continuar trabajando con esta lista? S/N: ");
...
//***********************************************************************
int reducirPorSueldo(Employee* pElement)
{
    int retorno = 0;
    if(pElement != NULL)
    {
        printf("\n Sueldo: %d", pElement->sueldo);
        if(pElement->sueldo < 15000 && pElement->sueldo > 0)
        {
            retorno = 1;
        }
    }
